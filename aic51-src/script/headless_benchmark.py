...
def target_matches(record: Any, target: dict[str, Any], point_tolerance: int) -> bool:
    ...
    if target["kind"] == "point":
        return any(abs(value - target["frame"]) <= point_tolerance for value in frames)
...
def metrics_for_results(results: list[Any], case: dict[str, Any], point_tolerance: int) -> dict[str, Any]:
    ...
    group_ranks = [group_match_ranks(judged, group, point_tolerance) for group in case["accepted_groups"]]
    if not group_ranks:
...
def run_text_variant(
    ...
    return {
        ...
        **metrics,
        ...
        "top_results": [
            serialize_result(record, rank, case, args.point_tolerance_frames)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Default exact-frame matching. Use a nonzero value only with documented source evidence.",
    )
...
def main() -> int:
    ...
    benchmark_contract = {
        ...
        "interval_endpoints": "inclusive source-frame counters",
        "point_tolerance_frames": args.point_tolerance_frames,
        "qa_semantics": "supporting-frame retrieval only; answer text is not graded",
    }
    params = {
        "nprobe": args.nprobe,
        "temporal_k": args.temporal_k,
