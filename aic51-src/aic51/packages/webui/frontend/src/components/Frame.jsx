...
function ScoreBreakdown({ scores }) {
  ...
  return (
    <div className="space-y-0.5">
      <div className="flex justify-between gap-3"><span>Final:</span><span className="font-bold text-yellow-400">{formatScore(scores.final)}</span></div>
      <div className="flex justify-between gap-3"><span>CLIP:</span><span className="font-bold text-green-400">{formatScore(scores.clip)}</span></div>
      <div className="flex justify-between gap-3"><span>OCR:</span><span className="font-bold text-blue-400">{formatScore(scores.ocr)}</span></div>
      <div className="flex justify-between gap-3"><span>ASR:</span><span className="font-bold text-purple-400">{formatScore(scores.asr)}</span></div>
    </div>
  );
}
...
export function FrameItem({
  ...
  const rawFinalScore = scores?.final ?? score;
  const numericFinalScore = Number(rawFinalScore);
  const hasFinalScore = Number.isFinite(numericFinalScore);

  const getScoreBorder = () => {
    if (!hasFinalScore) return "border-l-slate-300 border-l-4";
    if (numericFinalScore >= 0.8) return "border-l-emerald-500 border-l-4";
    if (numericFinalScore >= 0.5) return "border-l-amber-400 border-l-4";
  ...
  const filteredNearbyKeyframes = useMemo(
    () => nearbyKeyframes.filter((kf) => nearbySearchFilter ? String(kf).includes(nearbySearchFilter) : true),
  ...
  const handleNearbyScroll = (e) => {
    ...
    setNearbyVisibleCount((current) => Math.min(current + NEARBY_BATCH_SIZE, filteredNearbyKeyframes.length));
  };
    ...
    <>
      ...
      >
        ...
        >
          ...
          >
            {hasFinalScore ? `${(numericFinalScore * 100).toFixed(0)}%` : "score —"}
          </button>
        ...
        <div className="flex justify-between items-center mt-1 pt-1 border-t border-gray-100 overflow-hidden" onClick={(e) => e.stopPropagation()}>
          <div className="flex items-center gap-1 overflow-hidden shrink-0">
            <button onClick={onPlay} className="p-1 bg-blue-600 hover:bg-blue-700 text-white rounded-md transition-all shadow-sm hover:scale-105 active:scale-95 flex items-center justify-center shrink-0" title="Play video at frame">
              <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg>
            </button>
            <button onClick={() => (onSearchNearby ? onSearchNearby() : setShowNearbyModal(true))} className="p-1 bg-indigo-50 hover:bg-indigo-600 hover:text-white text-indigo-700 rounded-md border border-indigo-200 transition-all shadow-sm hover:scale-105 active:scale-95 flex items-center justify-center shrink-0" title="Explore nearby keyframes">
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M4 6h16M4 10h16M4 14h16M4 18h16" /></svg>
            </button>
            <button onClick={() => setShowZoomModal(true)} className="p-1 bg-teal-50 hover:bg-teal-600 hover:text-white text-teal-700 rounded-md border border-teal-200 transition-all shadow-sm hover:scale-105 active:scale-95 flex items-center justify-center shrink-0" title="Zoom keyframe image">
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0zM10 7v6m3-3H7" /></svg>
            </button>
            <button onClick={() => setShowOcrModal(true)} className="p-1 bg-amber-50 hover:bg-amber-600 hover:text-white text-amber-700 rounded-md border border-amber-200 transition-all shadow-sm hover:scale-105 active:scale-95 flex items-center justify-center shrink-0" title="Read on-screen OCR text">
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
            </button>
            {onSearchSimilar && (
              <button onClick={onSearchSimilar} className="p-1 bg-purple-50 hover:bg-purple-600 hover:text-white text-purple-700 rounded-md border border-purple-200 transition-all shadow-sm hover:scale-105 active:scale-95 flex items-center justify-center shrink-0" title="Search similar keyframes">
      ...
      {showDetailsModal && (
        <div onClick={() => setShowDetailsModal(false)} className="fixed inset-0 z-50 bg-black/75 backdrop-blur-sm flex items-center justify-center p-4 animate-fadeIn">
          <div className="w-full max-w-md bg-slate-950 text-white border border-slate-700 rounded-xl shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-3 py-2 border-b border-slate-800 bg-slate-900">
              <div><div className="text-xs font-bold">Candidate details</div><div className="text-[9px] font-mono text-slate-400">{id}</div></div>
              <button type="button" onClick={() => setShowDetailsModal(false)} className="px-2 py-1 text-xs bg-slate-800 hover:bg-red-600 rounded">✕</button>
            </div>
            ...
            <div className="px-3 pb-3">
              ...
              <div className="bg-black border border-slate-800 rounded p-2 text-[10px] font-mono text-white"><ScoreBreakdown scores={scores} /></div>
              <p className="mt-2 text-[9px] leading-tight text-slate-500">This panel is the secondary extension point for future evidence/provenance fields when those APIs are accepted. No unavailable fields are inferred here.</p>
            </div>
          </div>
        </div>
      )}
      ...
      {showZoomModal && (
        <div onClick={() => setShowZoomModal(false)} className="fixed inset-0 z-50 bg-black/90 backdrop-blur-md flex items-center justify-center p-4 animate-fadeIn">
          <div className="relative max-w-5xl w-full bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-2xl flex flex-col" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-2 bg-black flex items-center justify-center max-h-[85vh] overflow-hidden"><img src={thumbnail} alt={`${video_id}_${frame_id}_zoomed`} className="max-h-[80vh] w-auto object-contain rounded-lg shadow-lg" /></div>
          </div>
        </div>
      )}

      {showOcrModal && (
        <div onClick={() => setShowOcrModal(false)} className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-fadeIn">
          <div className="relative max-w-lg w-full bg-slate-900 text-white p-4 rounded-2xl shadow-2xl flex flex-col gap-3 border border-slate-800" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="bg-slate-950 border border-slate-800 text-slate-100 rounded-xl p-3.5 max-h-64 overflow-y-auto font-mono text-xs leading-relaxed whitespace-pre-wrap select-text shadow-inner">
              {loadingOcr ? (
                <div className="flex items-center gap-2 text-slate-400 italic"><div className="w-4 h-4 border-2 border-amber-500 border-t-transparent rounded-full animate-spin"></div><span>Fetching detected OCR text...</span></div>
              ) : ocrText ? ocrText : <span className="text-slate-500 italic">No on-screen OCR text detected for this frame.</span>}
            </div>
            <div className="flex justify-between items-center pt-1 border-t border-slate-800 text-xs font-mono">
              <div className="text-slate-400 text-[11px]">{scores?.ocr !== undefined && <span>OCR score: <strong className="text-amber-400">{formatScore(scores.ocr)}</strong></span>}</div>
              <button type="button" onClick={() => ocrText && navigator.clipboard.writeText(ocrText)} disabled={!ocrText} className="px-4 py-1.5 bg-amber-600 hover:bg-amber-500 disabled:bg-slate-800 disabled:text-slate-600 text-white font-bold rounded-lg text-xs">Copy Text</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showNearbyModal && (
        <div onClick={() => setShowNearbyModal(false)} className="fixed inset-0 z-50 bg-black flex flex-col text-white animate-fadeIn">
          <div className="flex flex-col h-full w-full bg-slate-950 overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="px-3 py-2 bg-slate-900 border-b border-slate-800 flex justify-between items-center shrink-0 gap-2">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <span className="text-[10px] text-gray-400 font-mono shrink-0">showing {Math.min(nearbyVisibleCount, filteredNearbyKeyframes.length)}/{filteredNearbyKeyframes.length}</span>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                <input type="text" placeholder="Jump/filter frame ID…" value={nearbySearchFilter} onChange={(e) => setNearbySearchFilter(e.target.value)} className="bg-slate-900 border border-slate-700 text-xs px-2 py-0.5 rounded text-white font-mono focus:outline-none focus:border-blue-500 w-40" />
            ...
            <div className="flex-1 overflow-y-auto p-1 bg-black" onScroll={handleNearbyScroll}>
              {loadingNearby ? (
                <div className="w-full h-full flex flex-col items-center justify-center gap-2 text-gray-400 font-mono text-sm py-20"><div className="w-8 h-8 border-4 border-blue-500 border-t-transparent rounded-full animate-spin"></div><span>Loading keyframe index for {video_id}...</span></div>
                ...
                <>
                    ...
                    {filteredNearbyKeyframes.slice(0, nearbyVisibleCount).map((kf) => {
                      ...
                      return (
                        <div
                          key={kf}
                          id={`nearby-kf-${kf}`}
                          onClick={() => isSelectedFrame ? removeSelected(kfId) : addSelected(kfId)}
                          ...
                          className={`relative aspect-video w-full bg-black rounded overflow-hidden cursor-pointer group hover:scale-[1.02] transition-transform ${
                            isSelectedFrame ? "ring-4 ring-emerald-500 z-10" : isCurrentTarget ? "ring-2 ring-blue-400" : "hover:ring-2 hover:ring-emerald-400/60"
                          }`}
                        >
                          <img src={`http://127.0.0.1:6900/api/files/${video_id}/${kf}`} alt={kf} loading="lazy" className="w-full h-full object-cover" />
                  ...
                  {nearbyVisibleCount < filteredNearbyKeyframes.length && <div className="text-center text-[10px] font-mono text-gray-500 py-3">Scroll to progressively mount more keyframes…</div>}
                </>
              )}
            </div>
          </div>
...
export function FrameContainer({ children }) {
  ...
  return (
    <ScoreOverlayContext.Provider value={showScoreOverlay}>
      <div className="flex justify-end mb-1 pointer-events-none">
        <button
          type="button"
          ...
          className={`pointer-events-auto px-2 py-1 rounded border text-[9px] font-mono font-bold shadow-sm ${
            showScoreOverlay ? "bg-slate-900 text-white border-slate-700" : "bg-white text-slate-700 border-slate-300 hover:bg-slate-50"
          }`}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-1">{children}</div>
