...
test("Search restores candidate navigation, triage, and quick-open shortcuts", () => {
  ...
  assert.deepEqual(getSearchShortcutAction({ code: "Digit1", key: "!", shiftKey: true }), { type: "quick-open", index: 0 });
  assert.deepEqual(getSearchShortcutAction({ code: "Digit0", key: ")", shiftKey: true }), { type: "quick-open", index: 9 });
...
test("Video keeps playback shortcuts and uses S for answer-frame selection", () => {
  ...
  assert.deepEqual(getVideoShortcutAction({ code: "Slash", key: "?", shiftKey: true }), { type: "focus-answer" });
