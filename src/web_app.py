...
from src.ocr_search import display_search_results, search_ocr_vintern, find_frame_idx_from_keyframe_id
from src.temporal_search import search_by_temporal
from src.transcript_search import transcript_search
from src.dinov3_search import recommend
from src.http_request import EventRetrievalClient
...
def render_submission_ui():
    ...
    if submission_type == "KIS":
        st.info("Format: video_id,start_time_ms,end_time_ms")
    elif submission_type == "QA":
        st.info("Format: answer,video_id,time_ms")
    elif submission_type == "TRAKE":
        st.info("Format: video_id,frame_id1,frame_id2,...")
        
    submission_content = st.text_area("Submission Content", height=100)
    submit_button = st.button("Submit to API")
    ...
    if submit_button:
        ...
        with st.spinner("Submitting to API..."):
            try:
                ...
                if submission_type == "KIS":
                    if len(parts) == 3:
                        video_id, start_ms, end_ms = parts
                        submission_response = client.submit_kis_answer(video_id, int(start_ms), int(end_ms))
                    else:
                        st.error("Invalid KIS format. Expected: video_id,start_time_ms,end_time_ms")

                elif submission_type == "QA":
                    if len(parts) == 3:
                        answer, video_id, time_ms = parts
                        submission_response = client.submit_qa_answer(answer, video_id, int(time_ms))
                    else:
                        st.error("Invalid QA format. Expected: answer,video_id,time_ms")

                elif submission_type == "TRAKE":
                    if len(parts) >= 2:
                        video_id = parts[0]
                        frame_ids_str = ",".join(parts[1:])
                        submission_response = client.submit_trake_answer(video_id, frame_ids_str)
                    else:
                        st.error("Invalid TRAKE format. Expected: video_id,frame_id1,frame_id2,...")

                if submission_response:
                    st.success("Submission successful!")
                    st.json(submission_response)
                else:
                    st.error("Submission failed. Check format or console for details.")
