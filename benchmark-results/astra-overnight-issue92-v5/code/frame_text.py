...
def main():
    ...
    reused_texts=unique_index(read_jsonl(ROOT.parent/'astra-overnight-issue92-v3/inputs/frame_text.jsonl'),field='frame_id')
    reuse_root=ROOT.parent/'astra-overnight-issue92-v3/outputs/frame-text/queries'
    ...
    for q in queries:
        ...
        rankings.append({'query_id':qid,'arms':{'control':control,'protected_full100_rrf60':rerank_frames(control,dict(zip(chosen,result['logits'])))}})
        timings.append(result)
    summary=evaluate(rankings,out,'protected_full100_rrf60')
