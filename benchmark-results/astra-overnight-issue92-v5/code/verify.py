...
for row in rankings:
    q=row['query_id'];control=row['arms']['control'];arm=row['arms']['protected_full100_rrf60']
    assert control==base[q]['arms']['temporal5s']
    ...
    original_comparison.append({'query_id':q,'arms':{'control':original[q]['arms']['control'],'protected_full100_rrf60':arm}})
verify=ROOT/'validation/recomputed';verify.mkdir(parents=True)
summary=evaluate(rankings,verify,'protected_full100_rrf60');saved=read_json(out/'summary.json')
for k,v in summary.items():assert v==saved[k],k
against=ROOT/'validation/vs-pr91';against.mkdir()
original_summary=evaluate(original_comparison,against,'protected_full100_rrf60');write(against/'summary.json',original_summary)
write(ROOT/'validation/accepted.json',{'status':'passed','queries':115,'scoreable':113,'source_hashes':len(sources),'cache_hashes':115,'reused_pairs_verified':reused,'new_pairs':new,'distinct_video':summary['metrics']['distinct_video'],'paired_vs_temporal5s':summary['paired']['distinct_video'],'paired_vs_pr91':original_summary['paired']['distinct_video']})
