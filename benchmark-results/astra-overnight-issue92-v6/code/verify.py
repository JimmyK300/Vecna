...
for row in rankings:
    q=row['query_id'];control=row['arms']['control'];arm=row['arms']['semantic80plus20']
    assert control==strongest[q]['arms']['protected_full100_rrf60']
    control=admitted[q]['arms']['admitted80plus20']
    assert control==projection(admit(base[q]['arms']['temporal5s']['frames'],provider[q]['arms']['grounded_dense']['frames']))
    assert control['frames'][:80]==base[q]['arms']['temporal5s']['frames'][:80]
    ...
    original_comparison.append({'query_id':q,'arms':{'control':original[q]['arms']['control'],'semantic80plus20':arm}})
verify=ROOT/'validation/recomputed';verify.mkdir(parents=True)
summary=evaluate(rankings,verify,'semantic80plus20');saved=read_json(out/'summary.json')
for k,v in summary.items():assert v==saved[k],k
against=ROOT/'validation/vs-pr91';against.mkdir()
original_summary=evaluate(original_comparison,against,'semantic80plus20');write(against/'summary.json',original_summary)
