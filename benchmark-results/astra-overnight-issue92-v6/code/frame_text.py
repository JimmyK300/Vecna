"""80 temporal5s plus20 semantic admissions through the same protected text ranker."""
import datetime
import json
import os
...
def main():
    ...
    timings=[];rankings=[]
    for q in queries:
        qid=q['query_id'];control=base[qid]['arms']['admitted80plus20']
        ...
        rankings.append({'query_id':qid,'arms':{'control':strongest[qid]['arms']['protected_full100_rrf60'],'semantic80plus20':rerank_frames(control,dict(zip(chosen,result['logits'])))}})
        timings.append(result)
    summary=evaluate(rankings,out,'semantic80plus20')
