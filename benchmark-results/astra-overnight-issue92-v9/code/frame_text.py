"""Novel-video semantic replacement through the same protected text ranker."""
import datetime
import json
import os
...
def main():
    ...
    base=unique_index(read_jsonl(ROOT/'inputs/admission.jsonl'))
    strongest=unique_index(read_jsonl(ROOT.parent/'astra-overnight-issue92-v5/outputs/frame-text/rankings.jsonl'))
    ...
    timings=[];rankings=[]
    for q in queries:
        qid=q['query_id'];control=base[qid]['arms']['video_preserving']
        ...
        if path.exists():
            ...
            extra=read_json(v6/'outputs/frame-text/queries'/f'{qid}.json')
            assert extra['query_text_sha256']==q['query_text_sha256']
            for fid,score in zip(extra['chosen'],extra['logits']):
                if fid in texts and extra_texts[fid]['text']==texts[fid]['text']:
                    if fid in reuse:assert reuse[fid]==float(score)
                    reuse[fid]=float(score)
        ...
        rankings.append({'query_id':qid,'arms':{'control':strongest[qid]['arms']['protected_full100_rrf60'],'video_preserving':rerank_frames(control,dict(zip(chosen,result['logits'])))}})
        timings.append(result)
    summary=evaluate(rankings,out,'video_preserving')
