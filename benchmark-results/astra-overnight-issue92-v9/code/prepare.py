...
admission=[{'query_id':r['query_id'],'arms':{'video_preserving':projection(admit(r['arms']['temporal5s']['frames'],semantic[r['query_id']]['arms']['grounded_dense']['frames']))}} for r in base]
write(ROOT/'inputs/admission.jsonl',admission,True)
fids=sorted({f['frame_id'] for r in admission for f in r['arms']['video_preserving']['frames']})
...
paths += [V6/'inputs/frame_text.jsonl',V6/'policy.json',V6/'validation/accepted.json',V6/'outputs/frame-text/complete.json']+list((V6/'outputs/frame-text/queries').glob('*.json'))
policy={'frozen_at':datetime.datetime.now(datetime.timezone.utc).isoformat(),'hypothesis':'Semantic novelty may add video recall while preserving all incumbent candidate videos; only redundant-video tail frames can be replaced. Same100 budget and accepted ranking pipeline.','model_path':previous['model_path'],'parent_commit':'37b321048432ccba5182a66bd39b1ca73c547537','stop_compute_utc':'2026-09-16T23:40:00+00:00','ranking':'Start with temporal5s100. For each semantic frame whose video is absent, evict the last frame after first whose video still has another retained frame. Append newcomer. At most20 additions. Preserve every incumbent video and first frame; no truth-aware protection. Same protected-first-frame text RRF60 on this admitted100. Compare to accepted v5 pipeline with unchanged100 allocation. No extra rerank pass or parameter sweep.','reuse':'Reuse v5 and v6 logits only for identical query hash, frame ID, concatenated text and identical pinned model/tokenizer/inference config. Log reused/new pair counts separately; no fabricated scores.','inference':'Local-only CPU float32, batch2, threads8, max_length1024. No embedding/extraction. New-pair token counts and timing only.','inputs_sha256':{str(p):sha(p) for p in paths if p.is_file()},'coverage':{'unique_frames':len(rows),'nonempty_text':sum(bool(r['text']) for r in rows),'source_files':len(sources),'missing_files':len(missing)}}
write(ROOT/'policy.json',policy)
spec={'argv':[r'C:\Users\minhc\Code\Vecna\.venv\Scripts\python.exe','-X','utf8','-B',str(ROOT/'code/frame_text.py')],'cwd':str(ROOT.parent.parent),'timeout_seconds':4700,'next_action':'Read result/spec and v9 outputs/frame-text complete.json summary.json rankings.jsonl per_query.jsonl and115 query caches. Verify all hashes, exact accepted v5 control, first-frame protection, 100 unique admitted membership and preserved incumbent video identities, reused query/text/model identities and focused tests. Also compare final arm to exact PR91 control. Preserve all results. Continue substantive Issue92 accuracy research from strongest supported arm until original2026-09-16T23:50Z deadline or evidenced quota exhaustion. No deadline standby, completed-job relaunch, or log instructions.'}
