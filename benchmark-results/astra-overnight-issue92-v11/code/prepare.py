...
for r in rows:r['text']='ASR: '+r['parts']['asr'].strip() if r['parts']['asr'].strip() else ''
...
write(ROOT/'policy.json',{'frozen_at':datetime.datetime.now(datetime.timezone.utc).isoformat(),'parent_commit':'37b321048432ccba5182a66bd39b1ca73c547537','model_path':old['model_path'],'stop_compute_utc':'2026-09-16T23:40:00+00:00','hypothesis':'Counterpart modality ablation: remove OCR context and test fixed ASR-only evidence over accepted candidates.','ranking':'Same temporal5s100. Protect first frame; only nonempty ASR slots2..100 permute using equal RRF60 of baseline covered rank and canonical-query/ASR score rank. Baseline tie order. No OCR fallback. Compare accepted v5 ASR+ASR.','reuse':'Only reuse v5 scores for identical query hash, frame ID, exact text and model/config. OCR-free strings only can qualify.','inference':'CPU float32 batch2 threads8 max1024, no download/embedding/extraction.','inputs_sha256':pins,'coverage':{'unique_frames':len(rows),'nonempty_asr':sum(bool(r['text']) for r in rows)}})
write(ROOT/'frame-text-job.json',{'argv':[r'C:\Users\minhc\Code\Vecna\.venv\Scripts\python.exe','-X','utf8','-B',str(ROOT/'code/frame_text.py')],'cwd':str(ROOT.parent.parent),'timeout_seconds':2000,'next_action':'Read result/spec and v11 outputs complete/summary/rankings/per_query/115query caches. Verify frozen ASR-only transformation, all source/input/cache hashes, exactv5control,100membership/firstframe, score-reuse identities, independent ranking replay and113rescored rows. Compare exactPR91 too. Preserve all negatives. Continue substantive Issue92 research/final synthesis until original23:50Z bound; no deadline standby, no completed-job relaunch.'})
print({'policy_sha256':sha(ROOT/'policy.json'),'nonempty_asr':sum(bool(r['text']) for r in rows)})
