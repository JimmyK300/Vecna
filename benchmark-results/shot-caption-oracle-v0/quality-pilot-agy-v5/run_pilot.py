...
SANDBOXES = Path('C:/Users/minhc/.codex/media-sandboxes/issue95-quality-v5')
...
def main():
    ...
    for q in IDS:
        ...
        for family in FAMILIES:
            previous = BASE/'quality-pilot-agy-v4'/'outputs'/f'{q}__{family}'
            if (previous/'record.json').exists():
                prior = json.loads((previous/'record.json').read_text(encoding='utf-8'))
                if prior.get('candidate_valid'):
                    assert sha(previous/'caption.txt') == prior['caption_sha256']
                    assert sha(previous/'request.txt') == prior['request_sha256']
                    prior['reused_from'] = str(previous)
            ...
            args = [str(AGY),'--model','gemini-3.8-flash-high','--effort','high','--dangerously-skip-permissions','--output-format','stream-json','--print-timeout','6m','--print',prompt]
            started = time.time()
            ...
            row = {'query_id':q,'family':family,'returncode':rc,'status':result.get('status'),'error':result.get('error'),'view_file_called':viewed,'unexpected_tools':unexpected,'elapsed_s':time.time()-started,'conversation_id':result.get('conversation_id'),'sandbox':str(sandbox),'clip_sha256':clips[q]['clip_sha256'],'request_sha256':sha(dest/'request.txt'),'caption_sha256':sha(dest/'caption.txt'),'usage':result.get('usage')}
            ...
            if unexpected:
                ...
                return 2
    save(ROOT/'terminal.json', {'state':'pilot_attempts_frozen_pending_quality_review','records':outcomes,'valid_count':sum(r['candidate_valid'] for r in outcomes)})
    return 0
