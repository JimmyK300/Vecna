...
Execution revision v4: v3 correctly targeted the clip but headless permission policy denied read_file, producing no caption. Per-run permission flag now matches the successful direct-view probe. Global settings unchanged; exact-path-only prompt and strict tool audit retained. All earlier failed attempts preserved.
